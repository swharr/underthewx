...
import { queryReadings, ReadingResolution } from "../services/storageService";
import { log } from "../utils/logger";

app.http("weatherHistory", {
  methods: ["GET"],
  route: "weather/history",
  authLevel: "anonymous",
  handler: async (req: HttpRequest, _ctx: InvocationContext): Promise<HttpResponseInit> => {
    const resolution = (req.query.get("resolution") ?? "hourly") as ReadingResolution;
    ...
    const rows = await queryReadings(resolution, hours);
    ...
    log("info", "weatherHistory served", { resolution, hours, count: data.length });
