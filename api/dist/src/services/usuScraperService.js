...
exports.fetchUsu = fetchUsu;
exports.fetchAllUsu = fetchAllUsu;
...
async function fetchUsu(stationKey) {
    const cfg = STATIONS[stationKey];
    const cached = (0, cache_1.cacheGet)(cfg.cacheKey);
    if (cached)
        return cached;
    const data = await (0, retry_1.withRetry)(async () => {
        ...
        if (!res.ok)
            throw new Error(`USU API ${res.status} for station ${cfg.id}`);
    ...
    (0, cache_1.cacheSet)(cfg.cacheKey, reading, TTL_MS);
    (0, logger_1.log)("info", `USU ${stationKey} API OK`, {
        tempF: reading.tempF,
...
//# sourceMappingURL=usuScraperService.js.map