...
function computeFrostRisk(tempF, dewPointF, solarWm2, windSpeedMph) {
    if (tempF <= 28)
        return "imminent";
    if (tempF <= 32 || dewPointF <= 32)
        return "imminent";
    if (tempF <= 34)
        return "high";
    ...
    if (tempF <= 38 || dewPointF <= 34 || radiativeFrost)
        return "moderate";
    if (tempF <= 42 || dewPointF <= 36)
        return "low";
    return "none";
}
function aggregate(inputs) {
    ...
    function norm(r) {
        return {
            ...r,
            tempF: (0, elevationService_1.normalizeTemp)(r.tempF, r.elevationFt),
            dewPointF: (0, elevationService_1.normalizeTemp)(r.dewPointF, r.elevationFt),
            pressureInHg: (0, elevationService_1.normalizePressure)(r.pressureInHg, r.elevationFt),
    ...
    function wavg(field, skipZero = false) {
        const contributors = normalized.filter((s) => {
            if (!s.normalized || s.weight === 0)
                return false;
            const val = s.normalized[field];
        ...
        return contributors.reduce((sum, s) => {
            return sum + s.normalized[field] * (s.weight / totalW);
        }, 0);
    }
    ...
    function estimateSoil() {
        ...
        if (candidatesTemp.length === 0 && candidatesMoist.length === 0)
            return {};
    ...
    const homeLocation = {
        ...
        feelsLikeF: (0, elevationService_1.feelsLike)(tempF, humidity, windSpeedMph),
    ...
    const stationMap = {};
    if (inputs.tempest)
        stationMap.tempest = { ...inputs.tempest, stale: isStale(inputs.tempest) };
    if (inputs.wunderground)
        stationMap.wunderground = { ...inputs.wunderground, stale: isStale(inputs.wunderground) };
    if (inputs.usu16)
        stationMap.usu16 = { ...inputs.usu16, stale: isStale(inputs.usu16) };
    if (inputs.usu1302734)
        stationMap.usu1302734 = { ...inputs.usu1302734, stale: isStale(inputs.usu1302734) };
    const now = new Date().toISOString();
    (0, logger_1.log)("info", "Aggregation complete", { frostRisk: homeLocation.frostRisk, tempF: homeLocation.tempF });
    return {
        timestamp: now,
        dataAgeSeconds: 0,
        homeLocation,
...
//# sourceMappingURL=aggregatorService.js.map